...
public class HMDStatusSender : MonoBehaviour
{
    ...
    void OnHMDMounted()
    {
        if (wasUnmounted)  // 只有在之前是Unmounted状态时，才进行操作
        {
    ...
    void OnHMDUnmounted()
    {
        if (!wasUnmounted)  // 只有在之前是Mounted状态时，才进行操作
        {
            StartCoroutine(HandleHMDUnmounted());
        }
    }
    ...
    private System.Collections.IEnumerator HandleHMDUnmounted()
    {
        ...
        if (wasUnmounted)  // 确认状态仍为Unmounted
        {
